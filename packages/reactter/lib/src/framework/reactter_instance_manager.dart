...
mixin ReactterInstanceManager {
  /// It's used to track whether instances are currently being built or not.
  /// It is initially set to `false`.
  /// When instances are being built, this flag is set to `true`,
  /// and when the building process is completed, it's set back to `false`.
  /// This flag is used internally within the `ReactterInstanceManager` mixin
  /// to ensure that instances are
  /// properly created and attached to `ReactterState` objects.
  @internal
  bool isInstancesBuilding = false;

  /// This list is used to store instances of `ReactterState` that have been collected during the
  /// building process of instances.
  ...
  ReactterInstance<T?>? _getAndCreateIfNotExtist<T extends Object?>([
    ...
    _createInstanceAndAttachToStates<T>(reactterInstance);
  ...
  void _createInstanceAndAttachToStates<T>(ReactterInstance reactterInstance) {
    final _isBusy = isInstancesBuilding;
    final _statesTemp = [if (_isBusy) ..._statesRecollected];

    isInstancesBuilding = true;

    if (_isBusy) _statesRecollected.clear();
    ...
    _statesRecollected.clear();

    if (_isBusy) _statesRecollected.addAll(_statesTemp);

    isInstancesBuilding = false;
  }
