...
- `child`: An optional <HT>`Widget`</HT> that does not rebuild when the dependency changes. This widget is sent through the arguments of <HT>`builder`</HT> function.
- <HM>`builder`</HM>: A function that builds the widget tree based on the dependency.
It receives the following arguments:
  - `context`: The <HT>`BuildContext`</HT> of the widget. A handle to the location of <HT>`ReactterProvider`</HT> in the widget tree.
  - `instance`: The instance of the <HT>`T`</HT> dependency. It is available only to default constructor.
  - `child`: The child widget that it receives from the `child` argument.
...
### Initializing with different modes
