...
- <HT>`ReactterProvider`</HT>: Default constructor that provides a despencency using a builder function.
  ...
  ReactterProvider<T>(
    ...
    Widget? child,
    Widget builder(BuildContext context, T instance, Widget? child)?,
  })
  ```
...
```dart title="counter_view.dart" collapse={1-18, 28-51} "ReactterProvider" "ReactterConsumer" "context.watch" "CounterController" "counterController" "counterController.increment" "counterController.decrement" "counterController.count"
