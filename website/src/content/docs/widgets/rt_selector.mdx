...
The <HT><a href="https://pub.dev/documentation/flutter_reactter/latest/flutter_reactter/RtSelector-class.html">`RtSelector`</a></HT> widget is a similarly to <HT>[`RtConsumer`](/reactter/widgets/rt_consumer)</HT>.
It obtains the dependency provided by the closest <HT>[`RtProvider`](/reactter/widgets/rt_provider)</HT> widget and allows you to select specific states to compute a value.
