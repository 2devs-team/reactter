...
- <HM>**`output`**</HM>: *(Optional)* The output function for the logger. Default is <HM>`dev.log`</HM>.
...
- **`observer`**: The observer to add like <HT>`RtStateObserver`</HT> or <HT>`RtDependencyObserver`</HT>.

### Usage

You can add a <HT>`RtStateObserver`</HT> to listen to the state lifecycle, e.g.:
...
Or add a <HT>`RtDependencyObserver`</HT> to listen to the dependency lifecycle, e.g.:
