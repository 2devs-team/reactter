...
States(<HT>`RtState`</HT>) like <HT>`Signal`</HT>, <HT>`UseState`</HT>, <HT>`UseAsyncState`</HT>, <HT>`UseReducer`</HT> and <HT>`UseCompute`</HT> can hold any type of Dart value, including objects.
