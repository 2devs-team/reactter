...
When a state is declared lazily, it is not automatically bound to the dependency. In such cases, you can use the <HT>[`Rt.lazyState`](/reactter/api/methods/state_management_methods/#rtlazystate)</HT> method to bind the state to the dependency, e.g.:
