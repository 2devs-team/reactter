...
<HT><a href="https://pub.dev/documentation/flutter_reactter/latest/flutter_reactter/BuildContextExtension/select.html" target="_blank">`BuildContext.select`</a></HT> is a method that computes a value whenever the selected states change and registers the current widget as a listener for these changes.
...
V context.select<T, V>(
  V computeValue(T instance, ReactterState select(ReactterState state)),
  [String? id],
);
```
