...
The `fullName` state is defined using <HT>[`UseCompute`](/reactter/api/hooks/use_compute)</HT> to compute the full name based on the values of `firstNameInput` and `lastNameInput`. This ensures the full name is automatically updated whenever either input changes.
