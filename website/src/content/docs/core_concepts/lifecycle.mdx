...
You can listen to the lifecycle events of a dependency by using the `Reactter.on` method of the event handler. e.g:
...
```dart title="counter_app.dart" collapse={1-100}
...
```
```dart title="main.dart" "Reactter.on" "ReactterDependency" "Lifecycle.unregistered" "Lifecycle.registered" "Lifecycle.initialized" "Lifecycle.destroyed"
import 'package:reactter/reactter.dart';
