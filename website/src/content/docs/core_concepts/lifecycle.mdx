...
You can listen to the lifecycle events of an instance (like a dependency or state) using <HM>`Rt.on`</HM> or <HM>`Rt.one`</HM> method of the event handler. e.g:
...
:::note
  The <HT>`RtDependency`</HT> is a generic class that takes the type of the dependency and optionally an `id`.
