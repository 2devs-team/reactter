...
- Hooks
  - [UseState](/hooks/use_state)
  - [UseAsyncState](/hooks/use_async_state)
  - [UseReducer](/hooks/use_reducer)
  - [UseCompute](/hooks/use_compute)
- Methods
  - [lazyState](/methods/to_manage_state/lazy_state)
  - [batch](/methods/to_manage_state/batch)
  - [untracked](/methods/to_manage_state/untracked)
...
<Aside type="tip">
  Learn about [Custom Hook](/core_concepts/custom_hook).
</Aside>
...
and Reactter offers two fundamental approaches for creating states: [Signal](/classes/signal) and Hooks.
...
Emits two events: `Lifecycle.willUpdate` firts and `Lifecycle.didUpdate` last.
...
Only emits the `Lifecycle.didUpdate` event.
- [bind](https://pub.dev/documentation/reactter/latest/reactter/ReactterState/bind.html): Binds the state to the instance.
- [unbind](https://pub.dev/documentation/reactter/latest/reactter/ReactterState/unbind.html): Unbinds the state from the instance.
- [dispose](https://pub.dev/documentation/reactter/latest/reactter/ReactterState/dispose.html): Disposes the state and its listeners.

Although thease methods are available, it's not necessary to use them directly, as Reactter handles state management for you.
...
The `bind`, `unbind` and `dispose` methods are normally used to manage the [lifecycle](/core_concepts/lifecycle) of a state.
...
<Aside type="tip">
  Learn about [Lifecycle](/core_concepts/lifecycles).
</Aside>
...
- Hooks
  - [UseInstance](/hooks/UseInstance)
- Methods
  * [register](/methods/to_manage_instance/register)
  * [lazyBuilder](/methods/to_manage_instance/lazy_builder)
  * [lazyFactory](/methods/to_manage_instance/lazy_factory)
  * [lazySingleton](/methods/to_manage_instance/lazy_singleton)
  * [create](/methods/to_manage_instance/create)
  * [builder](/methods/to_manage_instance/builder)
  * [factory](/methods/to_manage_instance/factory)
  * [singleton](/methods/to_manage_instance/singleton)
  * [get](/methods/to_manage_instance/get)
  * [delete](/methods/to_manage_instance/delete)
  * [unregister](/methods/to_manage_instance/unregister)
  * [destroy](/methods/to_manage_instance/destroy)
  * [find](/methods/to_manage_instance/find)
  * [isRegistered](/methods/to_manage_instance/is_registered)
  * [getInstanceManageMode](/methods/to_manage_instance/get_instance_manage_mode)


<Aside type="tip">
If you are using Flutter, go to [Rendering Control](/core_concepts/rendering_control) to learn how it manage instances through Widgets and BuildContext extension methods.
</Aside>
...
To comprehend this mechanism thoroughly, let's break down the process into six steps:

1. **Registration**: The first step involves registering the dependency into Reactter's context with the specified `id` and `mode` params.
The `Lifecycle.registered` event is emitted.
2. **Resolving**: When there is a request for a dependency, Reactter gets it according to `id` and the `mode` with which it was registered.
3. **Initialization**: If the dependency with/without `id` is not yet created, Reactter initializes it based on registry.
The following events are fired:
    - `Lifecycle.initialized`
    - `Lifecycle.willMount`
    - `Lifecycle.didMount`
4. **Usage**: The dependency is then used across the app as needed.
Some of these events may occur:
    - `Lifecycle.willUpdate`
    - `Lifecycle.didUpdate`
5. **Destroying**: When the dependency with/without `id` is no longer required, Reactter destroys it.
The following events are fired:
    - `Lifecycle.willUnmount`
    - `Lifecycle.didUnmount`
    - `Lifecycle.destroyed`
6. **Unregistration**: This is the final step in the process.
When the dependency with/without `id` is no longer required and depending on `mode`, Reactter unregisters it.
...
and to active it,set it in the `mode` argument of [`Reactter.register`](/methods/to_manage_instance/register) and [`Reactter.create`](/methods/to_manage_instance/create),
or use [`Reactter.lazyFactory`](/methods/to_manage_instance/lazy_factory),  [`Reactter.factory`](/methods/to_manage_instance/factory).
...
and to active it,
set it in the `mode` argument of [`Reactter.register`](/methods/to_manage_instance/register) and [`Reactter.create`](/methods/to_manage_instance/create),
or use [`Reactter.lazySingleton`](/methods/to_manage_instance/lazy_singleton), [`Reactter.singleton`](/methods/to_manage_instance/singleton).
...
<Aside type="tip">
  Use [`Reactter.destroy`](/methods/to_manage_instance/destroy) if you want to force destroy the instance and its register.
</Aside>
...
- Methods
  - [on](/methods/to_manage_event/on)
  - [one](/methods/to_manage_event/one)
  - [off](/methods/to_manage_event/off)
  - [offAll](/methods/to_manage_event/offAll)
  - [emit](/methods/to_manage_event/emit)
