...
1. **Registration**: This stage involves registering the dependency into Reactter's context with the specified `id` and `mode` params.
    ...
    For this, you can use the following methods:
      - [`register`](/methods/to_manage_instance/register)
      - [`lazyBuilder`](/methods/to_manage_instance/lazy_builder)(registers with `builder` mode)
      - [`lazyFactory`](/methods/to_manage_instance/lazy_factory)(registers with `factory` mode)
      - [`lazySingleton`](/methods/to_manage_instance/lazy_singleton)(registers with `singleton` mode)
...
If the dependency with/without `id` is not yet created, Reactter initializes it based on registry(this condition doesn't apply to `find` method).
    ...
    For this, you can use the following methods:
      - [`get`](/methods/to_manage_instance/get)
      - [`find`](/methods/to_manage_instance/find)(doesn't create the instance)
      - [`create`](/methods/to_manage_instance/create)(registers if not found in registry)
      - [`builder`](/methods/to_manage_instance/builder)(registers with `builder` mode if not found in registry)
      - [`factory`](/methods/to_manage_instance/factory)(registers with `factory` mode if not found in registry)
      - [`singleton`](/methods/to_manage_instance/singleton)(registers with `singleton` mode if not found in registry)
...
4. **Deleting**: When the dependency with/without `id` is no longer required, Reactter destroys it.
    ...
    For this, you can use the following methods:
      - [`delete`](/methods/to_manage_instance/delete)
      - [`destroy`](/methods/to_manage_instance/destroy)(delete & unregister)
