...
```dart title="counter_app.dart" /Counter(?!\u0060)(?!Page)/ "Counter()" /counter(?!\\.dart)/ "counter.increment" "counter.decrement" "counter.count" "ReactterProvider" "ReactterConsumer"
...
In example, we used the `ReactterConsumer` widget to observe the `count` property of the `counter` instance,
...
```dart title="counter_app.dart" collapse={1-25, 40-51} del={26-29} ins={30-35} /Counter(?!\u0060)(?!Page)/ /counter(?!\\.dart)/ "counter.increment" "counter.decrement" "counter.count" "ReactterProvider" "ReactterConsumer" "context.watch"
import 'package:flutter/material.dart';
import 'package:flutter_reactter/flutter_reactter.dart';
import 'counter.dart';
