...
<a href="https://pub.dev/documentation/reactter/latest/reactter/Memo-class.html" target="_blank">`Memo`</a> is a class callable with memoization logic which it stores computation results in cacbe, and retrieve that same information from the cache the next time it's needed instead of computing it again.
...
or `generic arguments`(learn more [here](#generic-arguments)).
:::
