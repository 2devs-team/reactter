...
---
import { HM, HN, HS, HT } from '@/components/Highlight/index.ts';

States(<HT>`ReactterState`</HT>) like <HT>`Signal`</HT>, <HT>`UseState`</HT>, <HT>`UseAsyncState`</HT>, <HT>`UseReducer`</HT> and <HT>`UseCompute`</HT> can hold any type of Dart value, including objects.
...
```dart title="main.dart" "Signal" "User" "ReactterWatcher" "user.value.name" "user.value.age" "user" mark={31}
...
To simplify this process, you can add the following method to the <HT>`User`</HT> class:
